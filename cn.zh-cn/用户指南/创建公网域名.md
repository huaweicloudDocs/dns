...
3.  在左侧树状导航栏，选择“域名解析 \> 公网解析”。
    ...
    **图 1**  公网域名列表<a name="fig6313755294"></a>  
    ![](figures/公网域名列表.png "公网域名列表")

4.  在域名所在行，单击域名的名称或者“操作”列的“解析”，可以为域名配置解析记录。

    在“解析记录”页面，可以通过“添加记录集”为域名配置解析记录，详细内容请参见[记录集管理简介](记录集管理简介.md)。

    **图 2**  域名解析记录<a name="fig626714020163"></a>  
...
4.  在页面右上角，单击“创建公网域名”。
    ...
    **图 3**  公网域名列表<a name="fig629633692918"></a>  
    ![](figures/公网域名列表-0.png "公网域名列表-0")

5.  在“创建公网域名”页面中，输入域名及相关参数。

    **图 4**  创建公网域名<a name="fig2442123065212"></a>  
...
7.  在“公网域名”页面的域名列表中，单击域名的名称或者“操作”列的“解析”，可以为域名配置解析记录。

    在“解析记录”页面，可以通过“添加记录集”为域名配置解析记录，详细内容请参见[记录集管理简介](记录集管理简介.md)。

    **图 5**  域名解析记录<a name="fig14474104617157"></a>  
    ![](figures/域名解析记录-1.png "域名解析记录-1")
